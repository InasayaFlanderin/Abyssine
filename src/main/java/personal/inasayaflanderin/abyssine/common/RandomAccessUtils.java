...
public final class RandomAccessUtils {
	...
	public static <D> void isSort(D[] array, Comparator<D> comparator, int start, int end) {
		
	}

	public static <D> void isSort(List<D> list, Comparator<D> comparator, int start, int end) {
		var isSorted = true;

		for(var i = 0; i < list.size() - 1; i++) if(comparator.compare(list.get(i), list.get(i + 1)) < 0) isSorted = false;
